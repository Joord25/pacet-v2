...
export function useUserManagement(): TrainerDetails[] {
  ...
  const data = useMemo(() => {
    ...
    const trainersWithDetails = trainers.map(trainer => {
      const assignedMembers = users.filter(u => u.role === 'member' && u.assignedTrainerId === trainer.id);
      ...
      const scheduled = currentMonthSessions.filter(s => s.status !== 'cancelled' && s.status !== 'pending').length;
      const attended = currentMonthSessions.filter(s => s.status === 'attended' || s.status === 'late').length;
